...
class AuctionApp:
    def __init__(self, root):
        ...
        self.username = ""  # Lưu tên người dùng đang đăng nhập
        self.current_screen_name = ""  # Tránh chuyển màn hình trùng lặp
    ...
    def queue_server_response(self, data):
        """Thread-safe: Đưa phản hồi vào Main Thread."""
        try:
    ...
    def clear_frame(self):
        """Xóa màn hình hiện tại một cách an toàn để tránh lỗi TclError."""
        ...
        if self.current_frame:
            try:
                # Xóa các widget con trước để dọn dẹp bộ nhớ
                for widget in self.current_frame.winfo_children():
                    widget.destroy()
                self.current_frame.destroy()
            except Exception as e:
                print(f"[GUI] Cảnh báo khi dọn dẹp Frame: {e}")
            finally:
                self.current_frame = None
    ...
    def show_dashboard(self):
        ...
        self.root.unbind('<Return>') # Hủy bind Enter login
    ...
    def handle_server_response(self, data):
        ...
        if msg_type == 999: # RECONNECTED
            ...
            self.root.title("Hệ thống Đấu giá Trực tuyến")
            # Khi Server restart, session cũ bị mất nên bắt buộc quay về Login
            self.show_login_screen()
        ...
        if msg_type == 802: # Login Success
            ...
            self.show_dashboard()
            self.show_toast(f"Chào mừng {self.username} quay trở lại!", bg="#2196F3")
            self.root.after(200, lambda: self.backend.send_command({"type": 201}))

        elif msg_type == 801: # Error
            error_msg = data.get("message", "Thao tác thất bại")

            # LOGIC KICK OUT: Khi tài khoản đăng nhập ở nơi khác
            if "dang nhap o noi khac" in error_msg:
            ...
            if hasattr(self, 'dashboard_frame') and self.current_screen_name == "dashboard":
...
if __name__ == "__main__":
    ...
    # Hàm đóng ứng dụng an toàn
    def on_closing():
        if messagebox.askokcancel("Thoát", "Bạn có muốn đóng ứng dụng không?"):
            app.backend.stop()
            app_root.destroy()
