...
void broadcast_to_room(int room_id, char *json_message) {
    ...
    for (int i = 0; i < MAX_USERS; i++) {
        ...
        if (users[i].fd != 0 && users[i].is_logged_in && users[i].current_room_id == room_id) {
            send_message(users[i].fd, json_message);
        }
    }
}
...
char* handle_bid(int fd, cJSON *json) {
    ...
    if (u->current_room_id == -1) return create_error_response(ERR_UNKNOWN, "You are not in any room");
    ...
    if (!room) return create_error_response(ERR_UNKNOWN, "Room error");

    // LOGIC KIỂM TRA GIÁ: Phải cao hơn giá hiện tại ít nhất 1 bước giá (hoặc chỉ cần > hiện tại)
    if (price <= room->current_price) {
        return create_error_response(ERR_BID_TOO_LOW, "Price must be higher than current price");
    }
...
char* handle_join_room(int fd, cJSON *json) {
    ...
    for (int i = 0; i < MAX_ROOMS; i++) {
        if (rooms[i].room_id == room_id && rooms[i].is_active) {
            ...
            cJSON_AddNumberToObject(notif, "type", S2C_GENERIC_OK); // Hoặc loại tin riêng nếu muốn
            char msg[100];
            sprintf(msg, "User %s joined the room", u->username);
